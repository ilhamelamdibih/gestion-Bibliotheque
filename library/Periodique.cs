...
namespace library
{
    public partial class Periodique : Form
    {
        ...
        public void MAJList()
        {
            ...
            DataTable table = db.table("select * from periodique");
            guna2DataGridView1.DataSource = table;
        }
        private void guna2Button9_Click(object sender, EventArgs e)
        {
            DataTable table = db.table(string.Format("SELECT * FROM periodique where titre like '%{0}%' and quantite>0", textBox2.Text));
            guna2DataGridView1.DataSource = null;
            guna2DataGridView1.DataSource = table;
            code = "";
        }
        ...
        private void guna2DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (index >= 0 && index < guna2DataGridView1.Rows.Count - 1)
            {
                ...
                DataTable table = db.table(string.Format("SELECT * FROM periodique where code='{0}' and quantite>0", r.Cells[0].Value.ToString()));
        ...
        private void guna2Button10_Click(object sender, EventArgs e)
        {
            if(textBox1.Text != "" && textBox2.Text != "")
            {
                ...
                if (code == "")
                {
                    DialogResult m = MessageBox.Show("Merci de selecionner une Periodique dans la liste", "Information", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                }
                else
                {
                    if (client == false)
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show("Client est bloqués", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                        else
                        {
                            int c = db.insert_update_delete(string.Format("insert into Emprunt (cin,code,type) values('{0}','{1}','{2}')", textBox1.Text, code, "Periodique"));
                            if (c == 0)
                            {
                            ...
                            else
                            {
                                ...
                                db.insert_update_delete(string.Format("UPDATE periodique set quantite=quantite-1 where code='{0}'", code));
