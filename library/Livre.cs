...
namespace library
{
    public partial class Livre : Form
    {
        ...
        public void MAJList()
        {
            ...
            DataTable table = db.table("select * from livre");
        ...
        {
            DataTable table = db.table(string.Format("SELECT * FROM livre where titre like '%{0}%' and quantite>0", textBox2.Text));
            guna2DataGridView1.DataSource = null;
            guna2DataGridView1.DataSource = table;
            code = "";
        }
        ...
        private void guna2DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (index >= 0 && index < guna2DataGridView1.Rows.Count - 1)
            {
                ...
                DataTable table = db.table(string.Format("SELECT * FROM livre where code='{0}' and quantite>0", r.Cells[0].Value.ToString()));
        ...
        private void guna2Button10_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "" && textBox2.Text != "")
            {
                ...
                if (code == "")
                {
                    DialogResult m = MessageBox.Show("Merci de selecionner un Livre dans la liste", "Information", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                }
                else
                {
                    if (client == false)
                    ...
                    else
                    {
                        ...
                        {
                            int c = db.insert_update_delete(string.Format("insert into Emprunt (cin,code,type) values('{0}','{1}','{2}')", textBox1.Text, code, "Livre"));
                            if (c == 0)
                            {
                            ...
                            else
                            {
                                ...
                                db.insert_update_delete(string.Format("UPDATE livre set quantite=quantite-1 where code='{0}'", code));
